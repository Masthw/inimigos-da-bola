-- Fix tally_match_votes type casting for user_id and awards
--
-- Issue: jsonb_array_elements extracts elem->>'user_id' as text, which caused:
-- ERROR: column "user_id" is of type uuid but expression is of type text
-- when inserting into match_awards. Also cast array elements to uuid/bigint
-- for proper type matching in leaderboard updates.
...
DECLARE
  ...
  v_result_a jsonb;
  v_result_b jsonb;
  v_craque_award_ids bigint[];
  v_craque_winners uuid[];
  v_player record;
...
BEGIN
  ...
  IF v_players IS NOT NULL AND array_length(v_players, 1) > 0 THEN
    -- Goleador
  ...
  IF v_awards IS NOT NULL THEN
    ...
    LOOP
      ...
      IF v_players IS NOT NULL AND v_votes IS NOT NULL THEN
  ...
  IF jsonb_array_length(v_awards_to_insert) > 0 THEN
    ...
    SELECT (elem->>'match_id')::uuid, (elem->>'user_id')::uuid, (elem->>'award_id')::bigint
  ...
  IF v_season_id IS NOT NULL AND v_players IS NOT NULL THEN
    ...
    -- Craque bonus
    IF v_awards IS NOT NULL THEN
      SELECT ARRAY_AGG(a.id) INTO v_craque_award_ids
      FROM unnest(v_awards) a
      WHERE a.is_voting_based = true AND a.name ILIKE '%craque%';
    END IF;

    IF v_craque_award_ids IS NOT NULL AND jsonb_array_length(v_awards_to_insert) > 0 THEN
      SELECT ARRAY_AGG((elem->>'user_id')::uuid) INTO v_craque_winners
      FROM jsonb_array_elements(v_awards_to_insert) elem
      WHERE (elem->>'award_id')::bigint = ANY(v_craque_award_ids);
    END IF;
