...
export function Modal({ open, onClose, title, icon, children, actions }: Readonly<ModalProps>) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      <button
        type="button"
        className="absolute inset-0 w-full h-full bg-black/60 backdrop-blur-sm cursor-default border-none outline-none appearance-none"
        onClick={onClose}
      ...
      <div className="relative bg-surface-container-high border border-outline-variant brutal-shadow rounded-xl w-full max-w-md animate-in">
        ...
        {actions && <div className="px-5 pb-5 flex flex-col sm:flex-row gap-3 justify-end">{actions}</div>}
      </div>
    </div>
  );
}
