    ...
    <section className="md:col-span-4 md:h-120">
      <div className="bg-surface-container rounded-xl border border-outline-variant p-stack-md h-full flex flex-col">
        ...
        <div className="space-y-4 flex-1 flex flex-col justify-center min-h-0">
          {loading && Array.from({ length: 3 }).map((_, i) => <div key={i} className="h-14 bg-surface-variant animate-pulse rounded-lg w-full" />)}
              ...
              return (
                ...
                >
                  <div className="flex flex-1 items-center gap-3 min-w-0 mr-2">
                    <span className={`font-mono text-label-bold ${isTop3 ? POS_CLASSES[pos - 1] : "text-on-surface-variant"} w-4 shrink-0`}>
                      {pos}
                    </span>
                    ...
                    {isCurrent && (
                      <span className="ml-1 px-2 py-0.5 bg-on-primary text-primary font-mono text-label-sm uppercase shrink-0">Você</span>
                    )}
                  </div>
        ...
        <Link
          to="/rankings"
          className="w-full mt-6 shrink-0 text-primary font-mono text-label-sm flex items-center justify-center gap-1 hover:underline"
        >
