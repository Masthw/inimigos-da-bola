...
interface FavoritePositionsModalProps {
  open: boolean; // <-- Propriedade adicionada para o TS parar de reclamar
  onClose: () => void;
}
...
export function FavoritePositionsModal({ open, onClose }: Readonly<FavoritePositionsModalProps>) {
  ...
  const handleSelect = async (position: Position) => {
    if (saving) return;
    const isSelected = isFavorite(position.id);
  ...
  // Agora usamos o open que vem do pai!
  if (!open) return null;

  return (
    <div
      className={`fixed inset-0 z-50 transition-colors duration-200 flex flex-col justify-end ${isClosing ? "bg-black/0" : "bg-black/60"}`}
      onClick={handleClose}
    >
      <div
        className={`w-full bg-surface-container-high rounded-t-2xl max-h-[85vh] flex flex-col transition-transform duration-200 ease-out ${
