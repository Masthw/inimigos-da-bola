...
export function LiveMatchPanel({ matchId, teamAName, teamBName, teamAScore, teamBScore, players, onRefresh }: Readonly<LiveMatchPanelProps>) {
  ...
  const handleOwnGoalSubmit = async () => {
    const result = await addOwnGoal(matchId, selectedOwnGoalTeam, null);
    if (result.error) {
      setError(result.error);
      return;
    }
  ...
  return (
    <div className="mt-4 p-4 bg-error-container/10 border border-error/30 rounded-xl">
      <div className="flex items-center gap-2 mb-4">
        ...
        <span className="font-mono text-label-bold uppercase tracking-widest text-error">Partida ao Vivo</span>
      </div>
      ...
      <div className="flex flex-wrap gap-2">
        ...
        <button
          ...
          disabled={busy}
          onClick={() => {
            setEditingScore({ teamA: teamAScore, teamB: teamBScore });
            setError(null);
      ...
      >
        <div className="space-y-4">
          <div>
            <label htmlFor="scorer-select" className="block font-mono text-label-sm text-on-surface-variant uppercase tracking-widest mb-2">
              Quem fez o gol?
            </label>
            <select
              id="scorer-select"
          ...
          <div>
            <label htmlFor="assist-select" className="block font-mono text-label-sm text-on-surface-variant uppercase tracking-widest mb-2">
      ...
      >
        <div className="space-y-4">
          ...
          <div>
            <p className="block font-mono text-label-sm text-on-surface-variant uppercase tracking-widest mb-2">Gol a favor de qual time?</p>
      ...
      >
        <div className="space-y-4">
          <p className="font-body text-on-surface-variant">Confirme o placar final da partida:</p>

          <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-3">
            <div>
              <label
                htmlFor="score-team-a"
                className="block font-mono text-label-sm text-on-surface-variant uppercase tracking-widest mb-2 text-center"
              >
                {teamAName}
              </label>
              <input
