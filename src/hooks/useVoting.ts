...
export function useVoting(matchId: string | undefined) {
  ...
  const fetchVotingData = useCallback(async () => {
    ...
    const [matchRes, playersRes, awardsRes, votesRes] = await Promise.all([
      supabase
        .from('matches')
        .select('id, team_a_name, team_b_name, team_a_color, team_b_color, team_a_score, team_b_score, voting_ends_at, status')
        .eq('id', matchId)
        .single(),
      supabase
        .from('match_players')
    ...
    const craqueExists = votingAwards.some((a) => a.name.toLowerCase().includes('craque'))
    if (!craqueExists) {
      const craqueVoteCounts: Record<string, number> = {}
      allVotes.filter((v) => v.award_id === -3).forEach((v) => {
        if (v.voted_user_id) {
          craqueVoteCounts[v.voted_user_id] = (craqueVoteCounts[v.voted_user_id] || 0) + 1
        }
      })
      votingAwards.unshift({ id: -3, name: 'Craque da Partida', isAutomatic: false, voteCounts: craqueVoteCounts })
    }
    ...
    setVotingData({
      ...
      players,
      votes,
      hasVotedCraque: votes.some((v) => v.awardId === -3 || (votingAwards.find((a) => a.name.toLowerCase().includes('craque'))?.id === v.awardId)),
    })
    setLoading(false)
  }, [matchId, user])
  ...
  const submitVote = useCallback(async (awardId: number, votedUserId: string | null) => {
    ...
    if (votedUserId === null) {
      ...
      const existingVote = votingData?.votes.find(
        (v) => v.awardId === awardId && v.votedUserId !== null
      )

      if (existingVote) {
        const { error } = await supabase
          .from('match_votes')
          .update({ voted_user_id: votedUserId })
          .eq('match_id', matchId)
          .eq('voter_user_id', user.id)
          .eq('award_id', awardId)
        setSaving(false)
        if (error) {
          setError('Erro ao atualizar voto')
          return false
        }
      } else {
        const { error } = await supabase
    ...
    await fetchVotingData()
    return true
  }, [matchId, user, votingData, fetchVotingData])
  ...
  const getVotedPlayers = useCallback((awardId: number) => {
    const playerIds = votingData?.votes
      ...
      .map((v) => v.votedUserId!) ?? []
    return playerIds.map((id) => votingData?.players.find((p) => p.userId === id)).filter(Boolean) as VotingPlayer[]
  }, [votingData])
